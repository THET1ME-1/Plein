...
class BackdropPicker(
    ...
    suspend fun next(current: Backdrop, dark: Boolean, weatherCode: Int?): Backdrop {
     ...
     */
    private suspend fun fromNetwork(dark: Boolean, weatherCode: Int?): Backdrop? {
        if (!Network.isOnline(context)) return null
        if (prefs.backdropWifiOnly && !Network.isUnmetered(context)) return null
        ...
        return if (queries == null) source.next(dark) else source.next(dark, queries)
    }
}
